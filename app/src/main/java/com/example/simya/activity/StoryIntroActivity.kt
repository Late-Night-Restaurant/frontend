...
import com.example.simya.server.story.InquiryStoryDetailResponse
...
class StoryIntroActivity : AppCompatActivity() {
    ...
    fun inquiryStoryDetail(){
        simyaApi.getStoryDetail(UserTokenData.accessToken,UserTokenData.refreshToken,intent.getLongExtra(HOUSE_ID,0))
            .enqueue(object: Callback<InquiryStoryDetailResponse>{
                override fun onResponse(
                    ...
                    if(response.body()!!.code == OK){
                        runOnUiThread {
                            // 프로필 주인정보
                            binding.tvStoryProfileNick.text = response.body()!!.result!!.masterProfile.nickname
                            binding.tvStoryProfileIntro.text = response.body()!!.result!!.masterProfile.comment
                                //Glide binding.civStoryProfileImage 프로필 이미지 추가
                            // 이야기집 정보
                            binding.tvIntroMainMenu.text = response.body()!!.result!!.houseInfo.category
